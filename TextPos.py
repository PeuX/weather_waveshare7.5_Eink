...
class TextPos(object):
    def __init__(self, autoUpdate = True):
        ...
        self.imageRed = Image.new('1', self.papirus.size, WHITE)
        self.autoUpdate = autoUpdate

    def AddText(self, text, x=0, y=0, size = 20, Id = None, color = 0):
        ...
        if Id not in self.allText:
            self.allText[Id] = DispText(text.decode('string_escape'), x, y, size, color)
    ...
    def removeImageText(self, Id):
        # prepare for drawing
        image = self.image if self.allText[Id].color == 0 else self.imageRed
    ...
    def WriteAll(self):
        ...
        self.papirus.display_frame(self.papirus.get_frame_buffer(self.image),self.papirus.get_frame_buffer(self.imageRed))
    ...
    def Clear(self):
        ...
        self.imageRed = Image.new('1', self.papirus.size, WHITE)
        self.allText = dict()
        self.papirus.display(self.papirus.get_frame_buffer(self.image),self.papirus.get_frame_buffer(self.imageRed))
